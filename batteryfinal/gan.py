...
for step in range(10000):
    ...
    for line in data[1:]:
        ...
        real = torch.FloatTensor([line[-2],line[-1]])
        real = predict(real)
        ...
        x_hat = alpha *line[-2] + (1 - alpha) *fake
    ...
    if step % 50 == 0:  # plotting
        ...
        plt.cla()
        model_name = "./model/ganAG"+str(step)+".pkl"
        torch.save(G, model_name)
        model_name = "./model/ganApre"+str(step)+".pkl"
        torch.save(predict, model_name)
        model_name = "./model/ganAD"+str(step)+".pkl"
